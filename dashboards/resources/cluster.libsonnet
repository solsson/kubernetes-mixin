    ...
    'k8s-resources-cluster.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.statPanel('sum(namespace:kube_pod_container_resource_requests:sum{%(clusterLabel)s="$cluster", resource="cpu"}) / sum(kube_node_status_allocatable{resource="cpu",%(clusterLabel)s="$cluster"})' % $._config)
        )
        .addPanel(
        ...
        .addPanel(
          ...
          g.statPanel('sum(kube_pod_container_resource_requests{%(clusterLabel)s="$cluster", resource="memory"}) / sum(kube_node_status_allocatable{resource="memory",%(clusterLabel)s="$cluster"})' % $._config)
        )
        .addPanel(
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.tablePanel(podWorkloadColumns + [
            ...
            'sum(namespace:kube_pod_container_resource_requests:sum{%(clusterLabel)s="$cluster", resource="cpu"}) by (namespace)' % $._config,
            'sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{%(clusterLabel)s="$cluster"}) by (namespace) / sum(namespace:kube_pod_container_resource_requests:sum{%(clusterLabel)s="$cluster", resource="cpu"}) by (namespace)' % $._config,
