    ...
    'k8s-resources-cluster.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.statPanel('sum(kube_pod_container_resource_requests_cpu_cores{%(clusterLabel)s="$cluster"}) / sum(kube_node_status_allocatable_cpu_cores{%(clusterLabel)s="$cluster"} OR kube_node_status_allocatable{resource="cpu",%(clusterLabel)s="$cluster")' % $._config)
        )
        .addPanel(
          g.panel('CPU Limits Commitment') +
          g.statPanel('sum(kube_pod_container_resource_limits_cpu_cores{%(clusterLabel)s="$cluster"}) / sum(kube_node_status_allocatable_cpu_cores{%(clusterLabel)s="$cluster"} OR kube_node_status_allocatable{resource="cpu",%(clusterLabel)s="$cluster")' % $._config)
        )
        .addPanel(
        ...
        .addPanel(
          ...
          g.statPanel('sum(kube_pod_container_resource_requests_memory_bytes{%(clusterLabel)s="$cluster"}) / sum(kube_node_status_allocatable_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_node_status_allocatable{resource="memory",%(clusterLabel)s="$cluster"))' % $._config)
        )
        .addPanel(
          g.panel('Memory Limits Commitment') +
          g.statPanel('sum(kube_pod_container_resource_limits_memory_bytes{%(clusterLabel)s="$cluster"}) / sum(kube_node_status_allocatable_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_node_status_allocatable{resource="memory",%(clusterLabel)s="$cluster"))' % $._config)
        )
      )
      .addRow(
        g.row('CPU')
