    ...
    'k8s-resources-cluster.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.statPanel('1 - sum(:node_memory_MemAvailable_bytes:sum{%(clusterLabel)s="$cluster"}) / sum(kube_node_status_allocatable_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_node_status_allocatable{resource="memory",%(clusterLabel)s="$cluster""})' % $._config)
        )
        .addPanel(
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.tablePanel(podWorkloadColumns + [
            ...
            'sum(kube_pod_container_resource_requests_cpu_cores{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_requests{%(clusterLabel)s="$cluster", resource="cpu"}) by (namespace)' % $._config,
            'sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{%(clusterLabel)s="$cluster"}) by (namespace) / sum(kube_pod_container_resource_requests_cpu_cores{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_requests{%(clusterLabel)s="$cluster", resource="cpu"}) by (namespace)' % $._config,
            'sum(kube_pod_container_resource_limits_cpu_cores{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_limits{%(clusterLabel)s="$cluster", resource="cpu"}) by (namespace)' % $._config,
            'sum(node_namespace_pod_container:container_cpu_usage_seconds_total:sum_rate{%(clusterLabel)s="$cluster"}) by (namespace) / sum(kube_pod_container_resource_limits_cpu_cores{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_limitss{%(clusterLabel)s="$cluster", resource="cpu"}) by (namespace)' % $._config,
          ], tableStyles {
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.tablePanel(podWorkloadColumns + [
            ...
            'sum(kube_pod_container_resource_requests_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_requests{%(clusterLabel)s="$cluster", resource="memory"}) by (namespace)' % $._config,
            'sum(container_memory_rss{%(clusterLabel)s="$cluster", container!=""}) by (namespace) / sum(kube_pod_container_resource_requests_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_requests{%(clusterLabel)s="$cluster", resource="memory"}) by (namespace)' % $._config,
            'sum(kube_pod_container_resource_limits_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_limits{%(clusterLabel)s="$cluster", resource="memory"}) by (namespace)' % $._config,
            'sum(container_memory_rss{%(clusterLabel)s="$cluster", container!=""}) by (namespace) / sum(kube_pod_container_resource_limits_memory_bytes{%(clusterLabel)s="$cluster"} OR kube_pod_container_resource_limits{%(clusterLabel)s="$cluster", resource="memory"}) by (namespace)' % $._config,
          ], tableStyles {
